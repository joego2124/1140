...
function tokenGenerator(line) {
  ...
  //   else if(token.name == "occupied" || "not-occupied")
  //   {
  //       token.type = "OCCUPANCY";
  //       token.data = tokens[0];
  //   }
      ...
      case "SET":
        ...
        data[1]
          ...
          .forEach((element) => {
            ...
            if (element.includes("{")) {
              obj.name = element.substr(0, element.indexOf("{")).toUpperCase();
...
function jsWriter(tokens) {
  var header =
    ...
    // -VARIABLE LINE
  var listener =
    "var blockRef = firebase.database().ref('GreenLine/');\n" +
  ...
  var updateSwitchState = "function updateSwitchState(data){\n"+
  ...
  var update = "function update(auth, switchState){\n" + "var updates = {};\n";

  var updateInner = "updates['GreenLine/{0}/Authority'] = auth[{1}];\n";
  var updateSwitchInner = "updates['GreenLine/{0}/SwitchState'] = switchState;\n";
    ...
    switch (tokens[i].type) {
      ...
      case "BLOCKS":
        ...
        for (j = 0; j < tokens[i].data.length; j++) {
          ...
          updateBlocks += String.format(updateBlockInner, j, tokens[i].data[j]);
        }
        console.log(blocks);
        break;
      case "BRANCH":
        ...
        for (j = 0; j < tokens[i].data.length; j++) {
          ...
          if (j == 0) {
            if (element.data.name.includes("blocks")) {
                ...
                var arg = element.type.includes('OCCUPANCY') ? "occupancy" : "authority";
