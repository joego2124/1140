...
function makeTrainSim(newTrainId) {
  ...
  Firebase.database()
    ...
    .on('value', (snapshot) => {
      train.blocknumber = snapshot.val();
    });
  Firebase.database()
    .ref(`/TrainList/${newTrainId}/PreviousBlock`)
    .on('value', (snapshot) => {
      train.previousblocknumber = snapshot.val();
    });
  Firebase.database()
    ...
    if (this.ebrake == true) {
      ...
      if (this.sbrake == true) {
        ...
        if (Math.abs(this.velocity) < 4) {
          ...
          Firebase.database()
            .ref(`/TrainList/${this.trainId}/Acceleration`)
            .set((this.velocity / Math.abs(this.velocity)) * -3.9);
        }
      } else {
        //no brakes
        var acc =
      ...
      Firebase.database()
        ...
        .once('value', (snapshot) => {
          ...
          if (beacon?.CurrentStation != 0) {
            if (
              this.NextStation == beacon.CurrentStation ||
              this.CurrentStation == 'Yard'
            ) {
              Firebase.database()
