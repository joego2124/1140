...
Button incSpeedButton = {13,0};
Button decSpeedButton = {12,0};
...
void IRAM_ATTR interrupt1()
{
  ...
  incSpeedButton.pressed = true;
}

void IRAM_ATTR interrupt2()
{
  Serial.println("------INTERRUPT-----");
  decSpeedButton.pressed = true;
}
...
void setup()
{
  ...
  if (!Firebase.RTDB.beginStream(&fbdoDownload, "/TC/CurrentSpeed"))
  {
    //Could not begin stream connection, then print out the error detail
    Serial.println(fbdoDownload.errorReason());
  }
  if (!Firebase.RTDB.beginStream(&fbdoUpload, "/TC/CurrentSpeed"))
  {
    //Could not begin stream connection, then print out the error detail
    Serial.println(fbdoUpload.errorReason());
  } 
  pinMode(incSpeedButton.PIN, INPUT_PULLUP);
  attachInterrupt(digitalPinToInterrupt(incSpeedButton.PIN), interrupt1, FALLING);
  pinMode(decSpeedButton.PIN, INPUT_PULLUP);
  attachInterrupt(digitalPinToInterrupt(decSpeedButton.PIN), interrupt2, FALLING); 
}
...
void loop()
{
  ...
  if(incSpeedButton.pressed)
  {
    long int start = millis();
            Serial.println(firebaseNum);

    currentSpeed = firebaseNum + 1;
    Firebase.RTDB.setInt(&fbdoUpload, "/TC/CurrentSpeed", currentSpeed);

    Serial.print("Milliseconds: ");
    Serial.println(millis() - start);
    incSpeedButton.pressed = false;
    
  }
  if(decSpeedButton.pressed)
  {
    long int start = millis();
            Serial.println(firebaseNum);
